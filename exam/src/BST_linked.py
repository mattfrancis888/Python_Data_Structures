...
class BST:
    ...
    def _insert_aux(self, node, value):
        ...
        if node is None:
            #Base case: add new node
            node = _BST_Node(value)
            self._count += 1
            inserted = True
        elif node._value > value:
            #General case: move to left subtree
            #have to assign node._left/_right
        ...
        if inserted:
            ...
            node._update_height()
        #node will be self._root after recursion calls
    ...
    def _is_identical_aux(self, node1, node2):
        ...
        if node1 is None and node2 is None:
            #Base case: Reached a bottom of the branch/tree.
            identical = True
