...
class Sorts:
    ...
    def radix_sort(a):
        ...
        -------------------------------------------------------
        Parameters:
          a - a Deque of base 10 integers (Deque)
        Returns:
          None
        -------------------------------------------------------
        """
        if len(a) > 0:
            ...
            for _ in range(passes):
                # Assign the Deque values to the appropriate bucket.
                Sorts._radix_split(a, buckets, digit)
                # Put the values back into the original Deque.
                Sorts._radix_merge(a, buckets)
                # Examine the next digit.
                digit += 1
        return

    @staticmethod
    def _radix_split(a, buckets, digit):
        """
        -------------------------------------------------------
        Splits the contents of a into buckets by digit base.
        Use: _radix_split(a, buckets, base)
        -------------------------------------------------------
        Parameters:
          a - an Deque of base 10 integers (Deque)
          buckets - a Deque of base empty Deques
          digit - the digit to extract (int)
        Returns:
          Contents of a are sorted into buckets by base digit.
        -------------------------------------------------------
        """
        p1 = 10 ** digit
        p2 = 10 ** (digit - 1)
        ...
        while a._front is not None:
            ...
            index = (current._value % p1) // p2
            # Put the number into the proper bucket.
            l = buckets[index]
            current._next = l._front
            l._front = current
        return

    @staticmethod
    def _radix_merge(a, buckets):
        """
        -------------------------------------------------------
        Merges buckets back into a.
        Use: _radix_merge(a, buckets)
        Merge from the last bucket to the first - takes advantage of
        the fact that items inserted into a linked Deque into the front
        ...
        Parameters:
          ...
          buckets - a Deque of base empty Deques
        Returns:
          Contents of a are sorted by base digit in buckets.
        -------------------------------------------------------
        """
        for i in range(len(buckets) - 1, -1, -1):
            l = buckets[i]

            while l._front is not None:
                current = l._front
                l._front = l._front._next
                current._next = a._front
                a._front = current
        return
    ...
    @staticmethod
    def gnome_r(a):
        Sorts.gnome_r_aux(a, a._front)
        return

    @staticmethod
    def gnome_r_aux(a, node):

        if node is not None:

            if node is a._front or node._value >= node._prev._value:
                Sorts.gnome_r_aux(a, node._next)
            else:
                # swap adjacent nodes
                a._swap(node, node._prev)

                if node is not a._front:
                    Sorts.gnome_r_aux(a, node._prev)
                else:
                    Sorts.gnome_r_aux(a, node._next)
        return
