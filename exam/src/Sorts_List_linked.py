...
class Sorts:
    ...
    def selection_sort(a):
        ...
        while unsorted is not None:
            ...
            max_node = unsorted #1
            previous = unsorted #1
            current = max_node._next #3
            
            #Find max val
            while current is not None: 
                
                if current._value > max_node._value: #if 3 > 1; 2 > 3 (max_node)
                    max_prev = previous  #None = 1; skip (Previous node for max_node)
                    max_node = current #1 = 3 ; skip 
                
                previous = current #1 = 3; 3 = 2 (Change to current in case you have another max_node and you need a max_prev)
                current = current._next #3 = 2; 2 = None (Move to next node)
            
            # Remove the max node from the list.
            Sorts.swaps += 1 

            if max_prev is None: #if 1 is None; (If max is the first node in the list)
                unsorted = max_node._next  #(Go to next node)
            else:
                max_prev._next = max_node._next # 3 = 2 #(Max prev points to max_node._next; why?)
            
            # Move the next max node to the front of the sorted list.
            max_node._next = a._front # 2 = None; (node after max_node is equal to None); why?
            a._front = max_node #a._front = 3 (Max node is now at the front of the list)
    ...
    def comb_sort(a):
        ...
        if n > 0:
            ...
            while gap > 1 or not done:
                done = True # Will go to loop if done is True and gap > 1
                ...
                i = 0
                prev_far = current # front, so 5
                far = current._next # front._next, so 3
                
                # Move to the far node for comparison.
                while i < gap - 1 and far is not None:
                    # i < 2 - 1
                    prev_far = far #Start at 3 #Need to get prev_far for _swap's parameters
                    far = far._next #Start at 1 #Need to get far to iterate from current to end of gap (far)
                    i += 1 #1
                    
                    #Without -1; the above code would not work, prev_far will be 1, far would be None

                while current is not None and far is not None:#Iterate till end of gap (far) is None
                    # While 5 is not None and 1 is not None
                    if current._value > far._value: #5 > 3
                        Sorts.swaps += 1
                    
                        # Look at parameters for _swap; Swap prev node's. So..[5,3,1] becomes [1,3,5] 
                        a._swap(previous, prev_far)
                        done = False 
                        #done = True when everything is ran trough again and everything sorted!
                        
                    # Increment all nodes.
                    #Say you had [5,3,1,2] but it swapped and became [1,3,5,2]
                    #Gap is 2, previous is None, prev_far is 3. far is 1
                    #Now it's....
                    #Gap is 2, previous is None, prev_far is 3, far is 5
                    #But you move to to the next item '3'.
                    prev_far = far #3 = 5 (move to next prev far)
                    far = far._next # 5 = 2 (move to next far)
                    previous = current # None = current (which is still 5? it would make sense if previous = self._front
                    #because we are trying to get the left node's prev for _swap)
                    current = current._next #5 = 2 
        return
        ...
        if len(a) > 0:
            ...
            for digit in range(passes):
                ...
                while not a.is_empty():
                    ...
                    buckets[index]._move_front_to_front(a) #_move_front_to_end would be ideal if it exist

                for index in range(len(buckets) - 1, -1, -1): #start at highest index
