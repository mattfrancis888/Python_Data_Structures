...
class Sorts:
    ...
    def comb_sort(a):
        ...
        while gap > 1 or not done:
            ...
            while j < n:
                #eg ;swap a[0] with a[7] in an array of 10 items
                #eg; then swap a[1] with a[8] in an array of 10 items; a[2] with a[9]; then since 10 < 10; exit loop
                if a[i] > a[j]:
                    Sorts._swap(a, i, j)
                    done = False
                i += 1
    ...
    def gnome_r_aux(a, pos):
        """
        -------------------------------------------------------
        Sorts an array using the Gnome Sort algorithm. Contents 
        of a up to index p are sorted.
        Use: gnome_sort(a)
        -------------------------------------------------------
        Parameters:
            a - an array of comparable elements (?)
